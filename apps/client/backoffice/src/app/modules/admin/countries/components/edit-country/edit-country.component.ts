...
export class EditCountryComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		if (this.dialogData.countryUuid) {
			this.editCountry = true;
			this.countriesService.getCountry(this.dialogData.countryUuid).subscribe((country) => {
	...
	saveCountry() {
		...
		if (this.editCountry) {
			this.countriesService
				.updateCountry(this.dialogData.country_uuid, countryDto)
				.subscribe((country) => {
					if (country) {
						...
						this.toaster.showToast(
							'error',
							this.transloco.translate('country.country-update-error'),
						);
					}
				});
		} else {
