...
class MembershipCard extends ConsumerWidget {
  ...
  const MembershipCard({super.key, required this.membership, required this.spent, required this.currency});
  ...
  Map<String, String?> _getMembershipLabels(BuildContext context, double? difference) {
    ...
    return {
      'title': translation(context).reachNextLevel(_membership),
      'description': translation(context).spendToReachNextLevel((difference ?? 0).toCurrency(currency), _membership),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final Map<String, String?> labels = _getMembershipLabels(context, goal - spent);

    return InformationCard(
        title: MultiStyledText(
          fullText: labels['title']!,
          defaultStyle: Theme.of(context).textTheme.headlineSmall,
          styledPlaceholders: {
            _membership: const TextStyle(color: AppColors.youngBlue)
          },
        ),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  colors: const [AppColors.youngBlue, AppColors.youngBlue, Colors.white, Colors.white],
