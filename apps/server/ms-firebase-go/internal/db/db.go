...
func InitDB() {
	...
	host := dotenv.GetEnv("POSTGRES_HOST")
	user := dotenv.GetEnv("POSTGRES_USER")
	password := dotenv.GetEnv("POSTGRES_PASSWORD")
	dbName := dotenv.GetEnv("FIREBASE_DB") // Change to MS DB name
	port := dotenv.GetEnv("POSTGRES_PORT")
	...
	// Setup custom join tables
	models.Notification{}.SetupJoinTable(db)
